...
bool CSADirRead::GetSubDirs(vector<CSADirRead::CSADirEntry> &dir_array, const CStdStringA &path)
{
	CStdStringA newPath;

	CStdStringA searchString;
	searchString = path;
	searchString+= "\\*.*";
   ...
   try 
   {
      ...
      HANDLE hFind;
      if ((hFind = FindFirstFileA(searchString, &FindFileData))!=INVALID_HANDLE_VALUE)
      {
...
// chop off trailing "\"
void CSADirRead::FormatPath(CStdStringA &path)
{
	CStdStringA inPath = path;
	inPath.TrimRight();
	CStdStringA tmp;
...
UINT CSADirRead::FindFiles(const CStdStringA & dir, const CStdStringA & filter, bool bIncludeFilesInFileList, bool bIncludeFoldersInFileList)
{
	// make sure the path ends in a single "\"
	CStdStringA baseName = dir;
	FormatPath(baseName);
	baseName+='\\';

	CStdStringA fullPath = baseName;
	fullPath += filter;
   ...
   HANDLE hFind;
	try 
	{
		if ((hFind = FindFirstFileA(fullPath, &FindFileData))!=INVALID_HANDLE_VALUE)
		{
