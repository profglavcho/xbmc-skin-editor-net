...
void CBaseTexture::Allocate(unsigned int width, unsigned int height, unsigned int format)
{
  ...
  int minpitch = 0;//g_Windowing.GetMinDXTPitch() intel use 128
  if (m_format & XB_FMT_DXT_MASK)
    while (GetPitch() < minpitch)
      m_textureWidth += GetBlockSize();

  if (0)//!g_Windowing.SupportsNPOT((m_format & XB_FMT_DXT_MASK) != 0))
  {
  ...
  CLAMP(m_textureWidth, g_pBitmapCreator.GetMaxWidth());
...
void CBaseTexture::Update(unsigned int width, unsigned int height, unsigned int pitch, unsigned int format, const unsigned char *pixels, bool loadToGPU)
{
  ...
  if (format & XB_FMT_DXT_MASK && !g_pBitmapCreator.SupportsDXT())
...
{
  //TODO
  /*if (URIUtils::GetExtension(texturePath).Equals(L".dds"))
  ...
  if (URIUtils::GetExtension(texturePath).Equals(L".jpg") || URIUtils::GetExtension(texturePath).Equals(L".tbn"))
  {
    CJpegIO jpegfile;
